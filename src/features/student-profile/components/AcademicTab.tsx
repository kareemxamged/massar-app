...
export default function AcademicTab({ userId }: { userId: string }) {
    ...
    const statCards = [
        { label: t('studentProfile.academicTab.totalExams', 'Total Exams Taken'), value: stats ? String(stats.totalExams) : '—', icon: BookOpen, color: '#60a5fa' },
        { label: t('studentProfile.academicTab.avgScore', 'Average Score'), value: stats ? `${stats.avgScore}%` : '—', icon: BarChart2, color: '#34d399' },
        { label: t('studentProfile.academicTab.passRate', 'Pass Rate'), value: stats ? `${stats.passRate}%` : '—', icon: TrendingUp, color: '#fb923c' },
        { label: t('studentProfile.academicTab.studyHours', 'Study Hours'), value: '42h', icon: Clock, color: '#a78bfa' },
        { label: t('studentProfile.academicTab.attendance', 'Attendance'), value: '96%', icon: CheckCircle, color: '#f472b6' },
    ];

    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', direction: i18n.dir() }}>
            ...
            <div className={styles.card} style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', flexWrap: 'wrap', width: '100%' }}>
                    ...
                    <div style={{ flex: 1 }}>
                        <h3 className={styles.cardTitle} style={{ marginBottom: '0.5rem' }}>{t('studentProfile.academicTab.performance', 'Academic Performance')}</h3>
                        <p className={styles.cardSubtitle}>{t('studentProfile.academicTab.performanceDesc', 'Computed from all submitted exam results.')}</p>
                        {stats && (
                            <div style={{ marginTop: '1rem', display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
                                {[
                                    { label: t('studentProfile.academicTab.exams', 'Exams'), val: stats.totalExams },
                                    { label: t('studentProfile.academicTab.avgScore', 'Average Score'), val: `${stats.avgScore}%` },
                                    { label: t('studentProfile.academicTab.passRate', 'Pass Rate'), val: `${stats.passRate}%` },
                                ].map(item => (
                ...
                {statCards.map(s => (
                    <div key={s.label} className={styles.statCard}>
                        ...
                        <div className={styles.statValue}>
                            <span dir="ltr" style={{ display: 'inline-block' }}>{s.value}</span>
                        </div>
            ...
            <div className={styles.card}>
                <div className={styles.cardHeader}>
                    <h3 className={styles.cardTitle}>{t('studentProfile.academicTab.achievements', 'Recent Achievements')}</h3>
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '0.75rem' }}>
                    {(stats?.avgScore ?? 0) >= 80 && (
                        <div style={{ display: 'flex', gap: '0.75rem', padding: '0.75rem', background: 'rgba(234,179,8,0.06)', borderRadius: '12px', border: '1px solid rgba(234,179,8,0.15)', alignItems: 'center' }}>
                            ...
                            <div>
                                <h4 style={{ color: 'white', margin: '0 0 4px 0', fontSize: '0.9rem' }}>{t('studentProfile.academicTab.highAchiever', 'High Achiever')}</h4>
                                <p style={{ color: 'var(--text-muted)', margin: 0, fontSize: '0.75rem' }}>{t('studentProfile.academicTab.highAchieverDesc', 'Average score above 80%')}</p>
                            </div>
                        </div>
                    )}
                    {(stats?.totalExams ?? 0) >= 5 && (
                        <div style={{ display: 'flex', gap: '0.75rem', padding: '0.75rem', background: 'rgba(59,130,246,0.06)', borderRadius: '12px', border: '1px solid rgba(59,130,246,0.15)', alignItems: 'center' }}>
                            ...
                            <div>
                                <h4 style={{ color: 'white', margin: '0 0 4px 0', fontSize: '0.9rem' }}>{t('studentProfile.academicTab.examVeteran', 'Exam Veteran')}</h4>
                                <p style={{ color: 'var(--text-muted)', margin: 0, fontSize: '0.75rem' }}>{t('studentProfile.academicTab.examVeteranDesc', 'Completed 5+ exams')}</p>
                            </div>
                        </div>
                    )}
                    {(!stats || stats.totalExams === 0) && (
                        <div style={{ color: 'var(--text-muted)', fontSize: '0.85rem', padding: '0.75rem', gridColumn: '1 / -1' }}>
                            {t('studentProfile.academicTab.noAchievements', 'No achievements yet — complete your first exam to unlock badges!')}
                        </div>
                    )}
                </div>
            </div>
